...
const App: React.FC = () => {
  ...
  // Global Historical Variances (Archive of all past price shifts)
  ...
  const bulkMarkStatus = (status: 'paid' | 'hold' | 'outstanding') => {
    ...
    addToast(`Successfully updated ${bulkSelection.size} audit records.`, 'success');
    clearSelection();
  };
  ...
  const exportSelectedToCSV = () => {
    ...
    csvContent += "Date,Invoice Number,Supplier,Total Amount,GST,Status\n";
      ...
      if (existingIdx > -1) {
        ...
        const updated = {
          ...
          history: [{
            ...
            note: `Market baseline sync from Inv #${invoiceNum}`
          }, ...item.history]
        };
        const next = [...prev];
        ...
        return [{
          ...
          history: [{
            ...
            source: 'audit' as const,
            invoiceNumber: invoiceNum,
            note: 'Initial Registry'
          }]
        }, ...prev];
      }
    });
    addToast(`Master Registry Sync: ${itemName} baseline is now $${newPrice.toFixed(2)}`, 'success');
  };
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of Array.from(files) as File[]) {
      ...
      try {
        ...
        let variancesDetectedCount = 0;
        data.items.forEach((item: any) => {
          const master = masterItems.find(m => m.supplierName === data.supplierName && m.name === item.name);
          if (master) {
             const backdated = isInvoiceBackdated(data.date, master.lastUpdated);
             if (!backdated && Math.abs(item.unitPrice - master.currentPrice) > 0.01) {
                variancesDetectedCount++;
                const diff = item.unitPrice - master.currentPrice;
                const direction = diff > 0 ? 'INCREASE' : 'DECREASE';
                addToast(`Price Alert: ${item.name} shifted to $${item.unitPrice.toFixed(2)} (${direction} of $${Math.abs(diff).toFixed(2)}) from baseline.`, 'warning');
             }
          }
        });

        if (variancesDetectedCount > 0) {
          addToast(`CRITICAL: ${variancesDetectedCount} active price shifts identified in Inv #${data.invoiceNumber}. Verify via the Variance Log.`, 'warning');
        }

        setSuppliers(prev => {
          const existing = prev.find(s => s.name === data.supplierName);
          const supDetails = { bankAccount: data.bankAccount, creditTerm: data.creditTerm, address: data.address, abn: data.abn, email: data.email, tel: data.tel };
            ...
            if (!exists) {
              next.push({
                ...
                history: [{
                  ...
                  source: 'audit', invoiceNumber: data.invoiceNumber, note: 'Auto-registered from latest audit'
                }]
              });
            }
          });
        ...
        addToast(`Successfully Processed: ${data.invoiceNumber}`, 'success');
      } catch (err: any) {
  ...
  return (
    <div className="min-h-screen flex bg-slate-50 font-sans text-slate-900 overflow-hidden print:bg-white relative">
      ...
      <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[1000] space-y-2 w-full max-w-md px-4 no-print pointer-events-none">
         {toasts.map(t => (
           <div key={t.id} className={`p-4 rounded-2xl shadow-2xl border flex items-start space-x-3 animate-in slide-in-from-top duration-300 pointer-events-auto ${t.type === 'success' ? 'bg-emerald-600 text-white' : t.type === 'warning' ? 'bg-rose-600 text-white shadow-rose-500/20 shadow-xl' : 'bg-slate-900 text-white'}`}>
              <div className="mt-0.5">
                {t.type === 'warning' ? <TriangleAlert size={18} /> : t.type === 'success' ? <CheckCircle2 size={18} /> : <Info size={18} />}
              </div>
              <div className="flex-1">
                <span className="text-[11px] font-black uppercase tracking-tight leading-tight block mb-0.5">{t.type === 'warning' ? 'Procurement Alert' : 'System Sync'}</span>
      ...
      <nav className={`w-72 bg-slate-900 text-slate-400 flex flex-col shrink-0 fixed inset-y-0 left-0 lg:sticky lg:top-0 h-screen z-[100] transition-transform duration-300 no-print ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <div className="flex-1 px-4 space-y-2 overflow-y-auto custom-scrollbar">
          ...
          <NavItem active={activeTab === 'variances'} onClick={() => {setActiveTab('variances'); setIsSidebarOpen(false);}} icon={HistoryIcon} label="Price Variance Log" alertCount={pendingVariances.filter(v => !v.isSynced).length} />
      ...
      <main className={`flex-1 overflow-y-auto p-4 lg:p-12 relative h-screen custom-scrollbar transition-all ${bulkSelection.size > 0 ? 'pb-40' : ''}`}>
        <header className="flex justify-between items-center mb-8 no-print sticky top-0 bg-slate-50/90 backdrop-blur-md py-4 z-[80] -mx-4 px-4 lg:-mx-12 lg:px-12">
          <div>
            <h1 className="text-2xl lg:text-3xl font-black text-slate-900 uppercase tracking-tighter">{activeTab.replace('-', ' ')}</h1>
            <p className="text-slate-400 text-[10px] lg:text-xs font-bold uppercase mt-1">Acme Supply Chain Solutions</p>
          </div>
        ...
        <div className="animate-in fade-in duration-500 max-w-7xl mx-auto space-y-10">
          ...
          {activeTab === 'dashboard' && (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                ...
                <StatCard label="Pending Shifts" value={pendingVariances.filter(v => !v.isSynced).length} icon={TriangleAlert} color="amber" />
                <StatCard label="Total GST" value={`$${stats.totalGst.toLocaleString()}`} icon={Banknote} color="emerald" />
                <StatCard label="Total Audited" value={stats.totalCount} icon={Files} color="slate" />
              </div>
              <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm p-8">
                 <h3 className="font-black text-slate-900 uppercase text-sm mb-6 flex items-center">
                   <Banknote size={18} className="mr-3 text-blue-600" /> Supplier Liability Ledger
                 </h3>
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {Object.keys(stats.supplierOutstanding).length === 0 ? (
                      <div className="col-span-full py-10 text-center text-slate-400 font-bold uppercase text-[10px]">No active liabilities on record</div>
                    ) : (
                      Object.entries(stats.supplierOutstanding).map(([name, amount]) => (
                        <div key={name} className="flex items-center justify-between p-5 bg-slate-50 rounded-3xl border border-slate-100 group hover:border-blue-200 transition-colors">
                           <div className="flex items-center space-x-4">
                              <div className="w-10 h-10 bg-slate-900 text-white rounded-xl flex items-center justify-center font-black text-xs uppercase shadow-lg shadow-slate-200">{name[0]}</div>
          ...
          {activeTab === 'variances' && (
            <div className="space-y-10">
                  ...
                  <div className="p-8 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row justify-between md:items-center gap-4">
                    <div>
                      <h3 className="font-black text-slate-900 uppercase text-sm flex items-center">
                        <TriangleAlert size={18} className="mr-3 text-rose-600" /> Current Audited Price Variances
                      </h3>
                      <p className="text-[10px] font-bold text-slate-400 uppercase mt-1">Pending market shifts from your most recent audits</p>
                    </div>
                  </div>
                    ...
                    <table className="w-full text-left min-w-[800px]">
                      <thead>
                        <tr className="bg-slate-50 text-[10px] font-black uppercase text-slate-400 border-b">
                          ...
                          <th className="px-8 py-5">Item & Vendor</th>
                          <th className="px-8 py-5 text-right">Market Shift</th>
                          <th className="px-8 py-5 text-center">Sync Baseline</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-50">
                        {pendingVariances.length === 0 ? (
                          <tr><td colSpan={5} className="py-20 text-center text-slate-400 font-bold uppercase text-[10px]">No material price variances detected in active audits</td></tr>
                        ) : (
                          pendingVariances.map((v, idx) => (
                            <tr key={`${v.invoiceId}-${idx}`} className={`hover:bg-slate-50/80 transition-all ${v.isSynced ? 'opacity-40 grayscale' : ''}`}>
                              <td className="px-8 py-5 text-[10px] font-bold text-slate-500">{v.date}</td>
                              <td className="px-8 py-5 font-bold text-slate-400 text-[10px]">{v.invoiceNumber}</td>
                              ...
                              <td className="px-8 py-5 text-right cursor-pointer group/item" onClick={() => v.masterItemId && setSelectedMasterItemId(v.masterItemId)}>
                                 <p className="font-black text-xs text-slate-900 group-hover/item:text-blue-600 underline decoration-slate-200 underline-offset-4">${v.newPrice.toFixed(2)}</p>
                                 <div className={`flex items-center justify-end text-[9px] font-black uppercase mt-1 ${v.variance > 0 ? 'text-rose-600' : 'text-emerald-600'}`}>
                                    ...
                                    {Math.abs(v.pct).toFixed(1)}% ({v.variance > 0 ? '+' : '-'}${Math.abs(v.variance).toFixed(2)})
                                 </div>
                              </td>
                              <td className="px-8 py-5 text-center">
                                {v.isSynced ? (
                                  <div className="flex items-center justify-center text-emerald-600"><CheckCircle2 size={20}/></div>
                                ) : (
                                  <button 
                                    onClick={() => updateMasterRate(v.supplierName, v.itemName, v.newPrice, v.invoiceNumber, v.date)}
                                    className="w-10 h-10 bg-white hover:bg-blue-600 hover:text-white rounded-xl flex items-center justify-center transition-all mx-auto border border-slate-200 shadow-sm active:scale-95"
                                  >
                                    <Check size={20}/>
                                  </button>
                                )}
              ...
              {/* History of Price Variance Section */}
              <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden">
                  <div className="p-8 border-b border-slate-100 bg-slate-50/50">
                    <h3 className="font-black text-slate-900 uppercase text-sm flex items-center">
                      <HistoryIcon size={18} className="mr-3 text-blue-600" /> Historical Price Movement Archive
                    </h3>
                    <p className="text-[10px] font-bold text-slate-400 uppercase mt-1">Complete log of all historical price adjustments recorded</p>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="w-full text-left min-w-[800px]">
                      <thead>
                        <tr className="bg-slate-50 text-[10px] font-black uppercase text-slate-400 border-b">
                          ...
                          <th className="px-8 py-5 text-right">Price Shift</th>
                          <th className="px-8 py-5 text-right">Impact</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-50">
                        {globalVarianceHistory.length === 0 ? (
                          <tr><td colSpan={5} className="py-20 text-center text-slate-400 font-bold uppercase text-[10px]">No historical price shifts archived yet</td></tr>
                        ) : (
                          globalVarianceHistory.map((h, idx) => (
                            <tr key={`${h.masterId}-${idx}`} className="hover:bg-slate-50/50 transition-all cursor-pointer" onClick={() => setSelectedMasterItemId(h.masterId)}>
                              <td className="px-8 py-5 text-[10px] font-bold text-slate-500">{new Date(h.date).toLocaleDateString()}</td>
                              <td className="px-8 py-5 font-bold text-slate-400 text-[10px] uppercase truncate max-w-[120px]">{h.invoiceNumber || 'Manual Sync'}</td>
                              <td className="px-8 py-5">
                              ...
                              <td className="px-8 py-5 text-right">
                                 <p className="font-black text-xs text-slate-900">${h.price.toFixed(2)}</p>
                              </td>
                              <td className="px-8 py-5 text-right">
                                 <div className={`flex items-center justify-end text-[9px] font-black uppercase ${h.variance > 0 ? 'text-rose-600' : 'text-emerald-600'}`}>
                                    ...
                                    {Math.abs(h.percentChange).toFixed(1)}% ({h.variance > 0 ? '+' : '-'}${Math.abs(h.variance).toFixed(2)})
                                 </div>
                              </td>
                            </tr>
                          ))
                ...
                <div className="p-4 lg:p-8 border-b border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center bg-slate-50/50 gap-4">
                  ...
                  {bulkSelection.size > 0 && (
                    <div className="flex items-center space-x-2 animate-in slide-in-from-right">
                       <span className="text-[10px] font-black uppercase text-blue-600 mr-4">{bulkSelection.size} Selected</span>
                       <button onClick={() => bulkMarkStatus('paid')} className="p-2.5 bg-emerald-100 text-emerald-700 rounded-xl hover:bg-emerald-600 hover:text-white transition-all"><CheckCircle2 size={18}/></button>
                       <button onClick={() => bulkMarkStatus('hold')} className="p-2.5 bg-amber-100 text-amber-700 rounded-xl hover:bg-amber-600 hover:text-white transition-all"><PauseCircle size={18}/></button>
                       <button onClick={() => bulkMarkStatus('outstanding')} className="p-2.5 bg-slate-200 text-slate-700 rounded-xl hover:bg-slate-900 hover:text-white transition-all"><X size={18}/></button>
                       <div className="w-px h-6 bg-slate-200 mx-2" />
                       <button onClick={exportSelectedToCSV} className="p-2.5 bg-blue-100 text-blue-700 rounded-xl hover:bg-blue-600 hover:text-white transition-all" title="Export CSV"><Download size={18}/></button>
                       <button onClick={printSelectedInvoices} className="p-2.5 bg-slate-900 text-white rounded-xl hover:bg-slate-800 transition-all" title="Print Selection"><Printer size={18}/></button>
                    </div>
                  )}
                </div>
                <div className="overflow-x-auto">
                          ...
                          <th className="px-6 py-5 w-12 text-center">
                            <button onClick={() => {
                              ...
                              if (bulkSelection.size === filtered.length) clearSelection();
                                ...
                                setBulkSelection(new Set(filtered.map(f => f.id)));
                              }
          ...
          {activeTab === 'suppliers' && (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {suppliers.map(sup => (
                <div key={sup.id} className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm hover:shadow-md transition-all group relative overflow-hidden">
                   ...
                   <h3 className="font-black text-slate-900 uppercase text-lg mb-4">{sup.name}</h3>
                   <div className="space-y-4 border-t border-slate-100 pt-6">
                      <div className="flex items-start text-[11px] text-slate-500 font-bold uppercase"><Hash size={14} className="mr-3 mt-0.5 opacity-40 shrink-0" /> <span className="text-slate-900">{sup.abn || 'NO ABN'}</span></div>
                      <div className="flex items-start text-[11px] text-slate-500 font-bold uppercase"><CreditCard size={14} className="mr-3 mt-0.5 opacity-40 shrink-0" /> <span className="text-slate-900">{sup.bankAccount || 'NO BANK INFO'}</span></div>
                      <div className="flex items-start text-[11px] text-slate-500 font-bold uppercase"><Phone size={14} className="mr-3 mt-0.5 opacity-40 shrink-0" /> <span className="text-slate-900">{sup.tel || 'NO CONTACT'}</span></div>
                      <div className="flex items-start text-[11px] text-slate-500 font-bold uppercase"><Clock size={14} className="mr-3 mt-0.5 opacity-40 shrink-0" /> Terms: <span className="text-slate-900 ml-1">{sup.creditTerm || 'NET 30'}</span></div>
                      <div className="flex items-start text-[11px] text-slate-500 font-bold uppercase"><MapPin size={14} className="mr-3 mt-0.5 opacity-40 shrink-0" /> <span className="text-slate-900 line-clamp-2">{sup.address || 'NO ADDRESS'}</span></div>
                   </div>
                </div>
              ))}
            </div>
        ...
        <div key={month} className="bg-white rounded-[2rem] border border-slate-200 overflow-hidden shadow-sm">
           <div className="px-8 py-6 bg-slate-900 text-white flex justify-between items-center">
              ...
              <div className="text-right">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Aggregate GST</p>
              ...
              <table className="w-full">
                 <thead>
                    <tr className="text-[9px] font-black uppercase text-slate-400 border-b border-slate-100">
                       <th className="px-6 py-4 text-left">Vendor Entity</th>
                       <th className="px-6 py-4 text-right">GST Portion</th>
                 ...
                 <tbody className="divide-y divide-slate-50">
                    {Object.entries(data.suppliers).map(([name, vals]: [string, any]) => (
                      <tr key={name} className="group hover:bg-slate-50 transition-colors">
                        <td className="px-6 py-5 font-black text-slate-900 text-xs uppercase">{name}</td>
                        <td className="px-6 py-5 text-right font-black text-emerald-600 text-xs">${vals.gst.toFixed(2)}</td>
                        <td className="px-6 py-5 text-right font-black text-slate-900 text-xs">${vals.total.toFixed(2)}</td>
                      </tr>
                    ))}
                 </tbody>
              </table>
...
const UploadView = ({ handleFileUpload, loading, progress }: any) => (
  <div className="max-w-4xl mx-auto py-10 lg:py-20 text-center animate-in fade-in duration-500 no-print px-4">
    ...
    <p className="text-slate-400 font-bold mb-12 uppercase tracking-widest text-[10px] lg:text-sm max-w-lg mx-auto">Upload PDF invoices. Price Guardian automatically audits against your anchored baseline, ignoring backdated entries.</p>
    <label className="group relative block cursor-pointer">
      <div className="border-4 border-dashed border-slate-200 rounded-[2.5rem] lg:rounded-[4rem] p-12 lg:p-24 transition-all group-hover:border-blue-500 group-hover:bg-blue-50 shadow-sm">
        <div className="flex justify-center space-x-4 mb-6"><Files size={40} className="text-slate-300 group-hover:text-blue-200 transition-colors" /><ImageIcon size={40} className="text-slate-300 group-hover:text-blue-200 transition-colors" /></div>
        <p className="text-slate-400 font-black uppercase text-[10px] lg:text-xs tracking-widest">Bulk Audit: Drag & Drop files here</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-10 space-y-10 custom-scrollbar">
         <div className="grid grid-cols-2 gap-8">
            <div className="min-w-0">
               <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Audit Source</p>
               <p className="font-black text-slate-900 text-lg uppercase leading-tight truncate">{invoice.supplierName}</p>
               <div className="mt-4 space-y-1">
                  <div className="text-[10px] font-bold text-slate-500 uppercase flex items-center"><Hash size={12} className="mr-2" /> ABN: {invoice.abn || 'N/A'}</div>
                  <div className="text-[10px] font-bold text-slate-500 uppercase flex items-center"><Mail size={12} className="mr-2" /> {invoice.email || 'N/A'}</div>
                  <div className="text-[10px] font-bold text-slate-500 uppercase flex items-center"><CreditCard size={12} className="mr-2" /> {invoice.bankAccount || 'N/A'}</div>
               </div>
            </div>
            <div className="text-right">
               <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Audit Liability</p>
               <p className="font-black text-3xl tracking-tighter text-slate-900">${invoice.totalAmount.toFixed(2)}</p>
               <p className="text-[10px] font-bold text-emerald-600 uppercase mt-2">GST Included: ${invoice.gstAmount.toFixed(2)}</p>
            </div>
         </div>
         <div className="space-y-4 pb-12">
           <h4 className="text-[10px] font-black uppercase text-slate-900 border-b pb-2 tracking-widest">Itemized Audit</h4>
           {invoice.items.map((item: any, idx: number) => (
             <div key={idx} className="p-5 bg-slate-50 rounded-3xl border border-slate-100 flex justify-between items-center transition-all hover:bg-white hover:shadow-lg hover:-translate-y-1 group">
                <div className="min-w-0 flex-1 pr-4"><p className="font-black text-xs uppercase text-slate-900 truncate">{item.name}</p><p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{item.quantity} x ${item.unitPrice.toFixed(2)}</p></div>
                <div className="text-right shrink-0">
                   <p className="font-black text-xs text-slate-900">${item.total.toFixed(2)}</p>
                   {item.percentChange !== 0 && (<div className={`flex items-center text-[9px] font-black uppercase mt-1 ${item.percentChange > 0 ? 'text-rose-600' : 'text-emerald-600'}`}>{item.percentChange > 0 ? <TrendingUp size={12} className="mr-1"/> : <TrendingDown size={12} className="mr-1"/>}{Math.abs(item.percentChange).toFixed(1)}%</div>)}
                </div>
             </div>
           ))}
         </div>
...
const SupplierEditModal = ({ supplier, onClose, onSave }: { supplier: Supplier, onClose: () => void, onSave: (updated: Supplier) => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-[400] flex justify-end no-print">
      ...
      <div className="relative w-full max-w-2xl bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50 sticky top-0 z-10">
          <div><h2 className="text-2xl font-black text-slate-900 uppercase tracking-tighter truncate">Vendor Intel Hub</h2><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{supplier.name}</p></div>
          <button onClick={onClose} className="p-2 hover:bg-white rounded-full text-slate-400"><X size={24}/></button>
        </div>
        <div className="flex-1 overflow-y-auto p-10 space-y-8 custom-scrollbar">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div><label className="text-[9px] font-black text-slate-500 uppercase mb-2 block">Official Trade Name</label><input type="text" value={edited.name} onChange={e => setEdited({ ...edited, name: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-xs font-bold focus:ring-2 focus:ring-blue-500 outline-none" /></div>
              <div><label className="text-[9px] font-black text-slate-500 uppercase mb-2 block">ABN / Registration</label><input type="text" value={edited.abn || ''} onChange={e => setEdited({ ...edited, abn: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-xs font-bold focus:ring-2 focus:ring-blue-500 outline-none" /></div>
           </div>
           <button onClick={() => onSave(edited)} className="w-full py-5 bg-slate-900 text-white rounded-2xl font-black uppercase text-[10px] hover:bg-slate-800 transition-all flex items-center justify-center space-x-2"><Save size={16} /><span>Sync Vendor Profile</span></button>
        </div>
      </div>
    </div>
  );
...
const MasterRateHistoryModal = ({ item, onClose }: any) => (
  <div className="fixed inset-0 z-[400] flex justify-end no-print">
    ...
    <div className="relative w-full max-w-2xl bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
      <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50 sticky top-0 z-10">
        <div><h2 className="text-2xl font-black text-slate-900 uppercase tracking-tighter truncate">{item.name}</h2><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{item.supplierName}</p></div>
        <button onClick={onClose} className="p-2 hover:bg-white rounded-full text-slate-400"><X size={24}/></button>
      </div>
      <div className="flex-1 overflow-y-auto p-10 space-y-12 custom-scrollbar">
         <div className="space-y-8 relative pl-4 pb-12">
            <div className="absolute left-[19px] top-4 bottom-12 w-0.5 bg-slate-100" />
            {item.history.map((h: PriceHistoryEntry, idx: number) => (
               <div key={idx} className="relative pl-12 group">
                  <div className={`absolute left-[-18px] top-1 w-9 h-9 rounded-full border-4 border-white shadow-lg flex items-center justify-center z-10 bg-slate-900 text-white`}><HistoryIcon size={16}/></div>
                  <div className="flex justify-between items-start mb-2">
                     <div>
                        <p className="text-lg font-black text-slate-900">${h.price.toFixed(2)}</p>
                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest flex items-center"><CalendarDays size={10} className="mr-1" /> {new Date(h.date).toLocaleDateString()}</p>
                        {h.invoiceNumber && <p className="text-[8px] font-bold text-blue-600 uppercase mt-1">Audit Link: Inv #{h.invoiceNumber}</p>}
                     </div>
                     {h.variance !== 0 && (<div className={`px-3 py-1 rounded-full text-[9px] font-black uppercase flex items-center space-x-1 ${h.variance > 0 ? 'bg-rose-50 text-rose-600' : 'bg-emerald-50 text-emerald-600'}`}>{h.variance > 0 ? <TrendingUp size={10}/> : <TrendingDown size={10}/>}<span>{h.variance > 0 ? '+' : ''}{h.percentChange.toFixed(1)}%</span></div>)}
                  </div>
               </div>
            ))}
         </div>
